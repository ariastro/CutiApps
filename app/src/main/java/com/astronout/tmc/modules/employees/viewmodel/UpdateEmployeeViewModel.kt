...
class UpdateEmployeeViewModel(application: Application): AndroidViewModel(application) {
    ...
    fun updateEmployee() {
        ...
        remoteRepository.postUpdateEmployee(empId.value!!, firstname.value!!, lastname.value!!, email.value!!,
            gender.value!!, dob.value!!, position.value!!, department.value!!, address.value!!,
            city.value!!, country.value!!, phoneNumber.value!!, annualLeaveRights.value!!, id.value!!, {
            setStatus(it.status)
        }, {
            logDebug(it)
        }, {
