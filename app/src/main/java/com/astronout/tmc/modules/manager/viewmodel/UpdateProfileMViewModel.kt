...
class UpdateProfileMViewModel(application: Application): AndroidViewModel(application) {
    ...
    fun uploadImage(){
        ...
        remoteRepository.postUpdateManagerAvatar(imageFile.value!!, {
    ...
    fun updateProfile() {
        setLoading(true)
        val remoteRepository = RemoteRepository(viewModelScope)
        remoteRepository.postUpdateManager(username.value!!, name.value!!, gender.value!!, dob.value!!,
            address.value!!, city.value!!, country.value!!, phoneNumber.value!!, User.managerId, {
                setStatus(it.status)
            }, {
                logDebug("updateProfileManagerError: $it")
            }, {
                setLoading(false)
            })
    }
