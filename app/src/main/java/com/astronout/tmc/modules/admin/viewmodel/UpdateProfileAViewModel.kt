...
class UpdateProfileAViewModel(application: Application): AndroidViewModel(application) {
    ...
    fun uploadImage(){
        ...
        remoteRepository.postUpdateAdminAvatar(imageFile.value!!, {
    ...
    fun updateProfile() {
        setLoading(true)
        val remoteRepository = RemoteRepository(viewModelScope)
        remoteRepository.postUpdateAdmin(username.value!!, name.value!!, gender.value!!, dob.value!!,
            address.value!!, city.value!!, country.value!!, phoneNumber.value!!, User.adminId, {
                setStatus(it.status)
            }, {
                logDebug("updateProfileAdminError: $it")
            }, {
                setLoading(false)
            })
    }
