...
class UpdateDepartmentViewModel(application: Application): AndroidViewModel(application) {
    ...
    fun updateDepartment() {
        ...
        remoteRepository.postUpdateDepartment(departmentName.value!!, departmentShortName.value!!,
            departmentCode.value!!, departmentId.value!!, {
            if (it.status) {
                setStatus(true)
            } else {
                setStatus(false)
            ...
            logDebug("#UpdateDepartmentError : $it")
        }, {
            setLoading(false)
        })
    }
